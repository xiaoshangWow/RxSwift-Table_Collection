...
class WTCollectionViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let items = Observable.just([
            SectionModel(model: "吴亦凡战队", items: ["那吾克热","王以太","blow fever"]),
            SectionModel(model: "热狗队", items: ["功夫胖","派克特","刘柏辛"]),
            SectionModel(model: "潘玮柏战队", items: ["ICE","周汤豪","艾热"]),
            ])
        ...
        self.collectionView.register(SectionHeader.self, forSupplementaryViewOfKind: UICollectionView.elementKindSectionHeader, withReuseIdentifier: "Section")
        ...
        let dataSource = RxCollectionViewSectionedReloadDataSource<SectionModel<String, String>> (
            ...
            configureSupplementaryView: { (dataSource, collectionView, kind, indexPath) in
                let section = collectionView.dequeueReusableSupplementaryView(ofKind: kind, withReuseIdentifier: "Section", for: indexPath) as! SectionHeader
